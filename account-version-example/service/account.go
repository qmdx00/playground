...
func (s *AccountSvc) UpdateUserAccountAndCreateFlow(userId int64, amount float64, bizNo string, flowType model.AccountFlowType) (*model.Account, *model.AccountFlow, error) {
	// HACK: 在主从架构下，需在主库执行事务，避免主从延迟导致的版本号不一致，导致大量失败的事务回滚
	tx, err := s.ds.Beginx()
	if err != nil {
		return nil, nil, err
	}
	...
	// HACK: 在高频写入场景下，需要加 FOR UPDATE，否则会导致大量失败的事务回滚
	// HACK: 在读多写少且低频写入的场景下，可以不加 FOR UPDATE，避免频繁的锁竞争（X 型行锁）
	account := new(model.Account)
	err = tx.Get(account, "SELECT * FROM account WHERE user_id = ? FOR UPDATE", userId) // HACK: 可只查询必要字段，使用联合索引来避免回表操作
	if err != nil {
		tx.Rollback()
		return nil, nil, err
	}
	...
	// HACK: 抛出错误，由客户端主动重试，此处也可以使用自动重试，但需要考虑重试次数和重试间隔
